...
public class E06Random {
	...
	public static void main(String[] args) {
		/*
		 ...
		 만약 Seed값이 동일 하다면 항상 동일한 패턴의 난수만 생성되는데 
		 이를 가르켜 가짜난수(Pseudo random number)라고한다
		 */
		System.out.println("### 난수생성1 : seed로 32를 사용");
		Random random1=new Random(32);
		for(int i=0 ; i<10 ; i++) {
			/*
			 nextInt의 인자값이 100이 전달되면 0~99사이의 난수 생성
			 */
		...
		System.out.println("### 난수생성3 : 항상 다른seed를 사용");
		...
		System.out.println("### 난수생성4 : 항상 다른seed사용, nextInt 인자없음");
		Random random4= new Random();
		//현재 시간을 밀리 초 단위로 반환하는 메소드
		random4.setSeed(System.currentTimeMillis());
		for(int i=0; i<10; i++) {
			/*
			 nextInt에 인자값이 없는경우에는 음수 양수가 혼용되어 난수가 생성된다.
			 */
