...
/*
 ...
 -반환값이 없다. 즉 특정 연산후 값을 반환할수 없다
 	따라서 void를 쓸수없다
 -그외는 메소드가 가진 모든 특성을 가지고있다(매개변수전달, 오버로딩)
 ...
 용도2:생성자내에서 멤버변수와 매개변수의 이름이 동일할깨 구분하기위해 사용한다
 this.변수=> 멤버변수
 변수=> 생성자의 매개변수
 단, 이름이 서로 다르면 this를 생략해도된다
 명시적인 코드작성을 위해 사용된다
 */
public class PersonConstructor {
	...
	/*
	디폴트 생성자
		: 해당클래스에 생성자를 정의하지 않은경우에 
		아래와 같은 매개변수, 실행부가 없는 생성자가 
		자동으로 삽입된다. 단, 생성자를 하나라도 
		정의하면 디폴트 생성자는 삽입되지않는다(주의)
	 */
	...
	public PersonConstructor(String name , int age) {
		/*
		this()는 생성자 내에서 다른생성자를 호출할때 사용한다
		단 생성자내에서만 사용가능하고 멤버메소드에서는 사용이불가능
		 */
		this(name,age,"미상");
	...
	public PersonConstructor(String _name, int age, String addr) {
		 ...
		 //매개변수와 멤버변수명이 다르면 this를 쓰지않아도된다 
		name= _name;
	...
	//멤버메소드 : 멤버변수를 초기화할 목적으로 정의한메소드
	void initialize(String name, int age, String addr) {
		//해당코드는 생성자 내에서만 사용할수있다.
		//this(name, age, "미상");
		//멤버변수를 구분하기위한 this는 사용할수없다
		this.name= name;
		this.age=age;
		this.addr=addr;
	}
