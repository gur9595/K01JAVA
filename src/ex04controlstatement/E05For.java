...
/*
 for문 :while 문과 같은 반복문 으로 초기값 조건식 증감식이 한줄에있어반복의
  횟수를 명시적으로 알수있는 반복문이다 그러므로 사용빈도가 가장높다
  ...
  형식
  	...
  	실행순서]
  	초기값->조건식확인->참일떄 실행->증감식->조건식확인
  	->반복실행->조건이 거짓일떄for문 탈출
 */
public class E05For {
	...
	public static void main(String[] args) {
		...
		/*
		 시나리오] for문을 이용하여 1~10사이의 정수중 
		 2의배수의 합을 구하는 프로그램을 작성학시오
		 */
		int total =0;
		//방법1: if문을  통한 구현
		for(int i=1; i<=10; i++) {
		...
		/*
		 for문 끝에; 을 사용하면 반복한 문장이 없는것으로 간주된다
		 */
		for(int j=0;j<=5;j++); {
			//for문과 상관없이 한번만 실행된다.
		...
		/*
		 for문위 초기식에서 선언된 변수 j눈 for문이 종료되는 순간
		 메모리에서 소멸된다. 여기서 사용된 변수j는 지역변수라고한다
		 지연변순ㄴ 해당 지역을 벗어나면 소멸되는 특징을 가지고있다.
		 */
		//System.out.println("위 for문에서 선언한 변수j="+j);
		System.out.println("위 main 함수 지역에서 선언한 변수 total="+total);
		/*
		 for문의 초기값을 외부에서 선언하면 해당변수는 for문안쪽에서 사용이 가능하다 
		 즉 아래에서 선언한i는 main함수의 지역변수가 된다
		 */

		int i=0;
		for(;i<=5;i++) {
			System.out.println("for문안에서의 i값="+i);
		}
