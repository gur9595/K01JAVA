...
/*
 ...
 -Java에서는 main메소드는 무조건 public static void로
 	선언하기로 약속 되어있다
 -static 으로 선언된 메소드에는 static으로 선선된 메소드만
 	호출할 수 있다
 -메소드 호출후 반환값은 항상 호출한 위치로 반환된다
 	이때 메소드는 메모리에서 소멸
 -변환값이 없는 void 형메소드도 실행이 완료되면 호출한 위치로 되돌아간다
 */
public class E01MethodBasic {
	/*
	 메소드 정의1: 반환타입O, 메개변수O 의 형태로 정의
	 -> 두개의 정수를 연달받아 합의 결과를 반환한다
	 */
	...
	/*
	 메소드 정의2: 변환타입x, 매개변수x 형태로 정의
	 -> 전당되는 값없이 이름을 입력받은후 namePrint()함수를 호출한다
	...
	public static void main(String[] args) {
		...
		/*
		 메소드를 호출할꺠는 함수의 원형을 그대로 기술하면 된다.
		 매개변수가 있다면 갯수만큼 포함 시켜줘야한다.
