...
/*
 	...
 	:예외 발생 여부와 상관없이 try문으로 진입했을때
 	반드시 실행해야 하는 코드가 있는경우 기술하는 블럭이다
 	즉 try문으로 진입되지 않는다면 finally절은 실행되지 않는다
 	1)try~catch : 예외를 직접 처리할때
 	2)try~catch~finally : 예외를 직접 처리후 실행할문장이있을때
 	3)try~finally : 예외는 외부로 던지고 예외 발생과 상관없이 실행할 문장이 있을떄
 */
public class Ex05Finally {
	...
	static void tryCatchFinally() {
		...
		try {
			...
			/*
			 try문안에서 exit(0)를 실행하게되면 프로그램 자체가 종료
			 finally구문은 실행되지 않는다
			 */
			System.exit(0);
		}
