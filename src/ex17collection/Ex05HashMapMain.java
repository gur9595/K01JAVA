...
public class Ex05HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 객체저장
		 	: 객체저장시 기존에 저장된 동일한 key값이 존대하면
		 	이전 객체가 반환된다. 만약 처음이라면 null값이 반환된다
		 */
		System.out.println("name이라는 키값으로 저장된 이전듸 값: "
		...
		/*
		 4] 중복저장
		 :기존에 입력된 name이라는 key값이 있으므로 "홍길동"출력
		...
		/*
		 5-2] 키값을 모를때 출력하기
		 	...
		 	Set계열의 컬렉션 형태로 가져온다
		 	2. Map계열의 컬렉션에는 처음부터 확장 for문을 사용하는것이 불가능하다
		...
		/*
		 5-3]반복자를 통한 출력
		 		:iterator를 통해 반복할떄도 keySet()으로 키값을 
		 		먼저 얻어온후 사용할 수 있다
		 */
		...
		/*
		 5-4] Value값만 얻어올때 : values() 메소드사용
		 */
		System.out.println("[value갑들만 출력하기]");
		...
		/*
		 6]존재유무판단
		 		: 존재할 경우 true를 반환한다.
		 */
		System.out.println(map.containsKey("name")?"name키가있다":"name키가 없다");
		System.out.println(map.containsKey("account")?"account키가있다":"account키가 없다");
		System.out.println(map.containsValue("남자")?"남자 있다":"여자 없다");
		System.out.println(map.containsValue("여자")?"여자 있다":"남자 없다");
