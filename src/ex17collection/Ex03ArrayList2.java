...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		Iterator<Student>it2= list2.iterator();
			/*
		 위와 동일한 문장임 next()메소드를 통해 객체를 가져온후 show매소드로 객체가져옴
		 */
		while(it2.hasNext()) {
		...
		for(int i=0 ; i<list2.size() ; i++) {
			//리스트 컬렉션에 인덱스로 접근할때는 get메소드를 사용
		...
		//인덱스를 통한 삭제: 삭제성공시 해당 객체가 반환
		System.out.println("삭제된 객체의 이름: "+list2.remove(2).name);
		//인스턴스 참조값을 통한삭제 : 삭제 성공시 boolean 값이 반환된다.
		list2.remove(st1);
