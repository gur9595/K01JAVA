...
/*
 HashSet 컬랙션: Set계열의 인터페이스를 구현한 컬랙션으로
 -객체가 순서없이 저장된다
 -객체의 중복저장을 기본적으로 허용하지 않는다
 	단 기본클래스가 아닌 새롭게 정의한 클래스 라면 
 	hashCode,equals메소드를 적절히 사용해 오버라이딩 해야한다
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/*
		 3] 객체저장
		 :add()메소드를 통해 객체가 정상적으로 저장되면 true반환
		 */
		...
		/*
		 5-1] 기본 클래스형 객체 중복저장
		 기본클래스의 경우에는 별도의 오버라이딩 없이 중복이 제거됨
		 따라서 아래의 경우는 false반환
		 */
		...
		/*
		 개발자가 정의한 객체 중복저장
		 :직접 정의한 클래스인 경우 hashCodr equals메소드를
		 오버라이딩 하지 않으면 중복저장이 허용된다
		 아래의졍우 저장에 성공하여 true반환
		 */
		...
		while(itr.hasNext()) {
			...
			if(object instanceof String) {
				...
				System.out.println("Interger타입: "+object);
