...
/*
 추상클래스 
 =상속을 못정으로 제작되므로 객체생성을 허용하지 않는다
 -추상클래스는 대부분 추상메소드를 포함하고있으므로
 이를 상속받는 클래스는 반드시 추상메소드를 오버라이딩 해야한다
 -클래스의 상속관계에서의 설계도 역할을한다
 -객체생성은 할수 없으나 그 외 참조변수 상속등의 모든 기능을 가지고있다
...
abstract class PersonalNumberStorageAbs{
	public abstract void addPersonlInfo(String juminNum, String name);
...
/*
 재공된 추상클래스를 상속하여 사람의 정보를 저장하는 기능과
 검색의 기능을 구현한 클래스
 */
