...
/*
 ...
 -정적변수는 JVM에 의해 프로그램이 로드되는 시점에서 초기화된다
 -main메소드가 실행되기전에 이미 초기화 되므로 main메소드 
 실행시 별도의 선언없이 변수를 사용할수있다
...
class MyStatic{
	/*
	 [멤버변수] 인스턴스형 멤버변수는 반그시 클래스의 객체를
	 생성한후 참조변수를 통해 접근해야한다.
	 */
	int instanceVar;
	/*
	 [정적멤버변수] 객체를 생성할 필요없이 클래스명으로 바로 접근이가능하다
	...
	/*
	 [정적멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할수없다
	 정적멤버만 사용이 가능하다
	 (main메소드에서는 static으로 선언된 메소드만 호출할수있었음을 기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 방법2: 인스턴스변수처럼 참조변수를 통해 접근할수있다.
		 	단, 이경우는 정적변수를 인스턴스변수로 착각할수있으므로 
		 	권장하지않는다.
		 */
